...
(function() {
	...
	module.exports = function(robot) {
		...
		robot.respond(/@?(.+) (has) (["'\w: -_]+) (role)/i, (msg) => {
			let _ref, _ref1, myRoles, name, newRole, user;
			...
			if ((_ref = name.toLowerCase()) !== "" && _ref !== "who" && _ref !== "what" && _ref !== "where" && _ref !== "when" && _ref !== "why") {
				...
				if (user == null) {
					return msg.reply(`${String(name)} does not exist`);
				}
				user.roles || (user.roles = []);
				if (__indexOf.call(user.roles, newRole) >= 0) {
					return msg.reply(`${String(name)} already has the '${newRole}' role.`);
				}
				if (newRole === "admin") {
		...
		robot.respond(/@?(.+) (doesn't have|does not have) (["'\w: -_]+) (role)/i, (msg) => {
			let _ref, _ref1, myRoles, name, newRole, role, user;
			name = msg.match[1].trim();
			newRole = msg.match[3].trim().
				toLowerCase();
			if ((_ref = name.toLowerCase()) !== "" && _ref !== "who" && _ref !== "what" && _ref !== "where" && _ref !== "when" && _ref !== "why") {
		...
		robot.respond(/(?:what|which) role(s?) does @?(.+) have\?*$/i, (msg) => {
			...
			user.roles || (user.roles = []);
			if (_ref = user.id.toString(), __indexOf.call(admins, _ref) >= 0) {
				return msg.reply(`${String(user.name)} has admin role and thus has all the roles :all_the_things: `);
			} else if (user.roles.length === 0) {
				return msg.reply(`${String(user.name)} doesn't have any roles :forever_alone:.`);
			}
			return msg.reply(`${String(user.name)} has the following roles: ${user.roles.join(", ")}.`);

		});
		return robot.respond(/who has admin role\?*$/i, (msg) => {
			let _i, _len, admin, adminNames, user;
			adminNames = [];
			for (_i = 0, _len = admins.length; _i < _len; _i++) {
				admin = admins[_i];
				user = robot.brain.userForId(admin);
				if (user != null) {
					adminNames.push(`${String(user.real_name)} (@${user.name})`);
				}
			}
