...
(function() {
	let analyze, negativity, positivity, sentimental;
	...
	module.exports = function(robot) {
		return robot.respond(/check (on )?(.*)/i, (msg) => {
			let data, raw, scores, user, username;
			username = msg.match[2];
			raw = robot.brain.get("hubot-sentimental");
			if (raw) {
				scores = JSON.parse(raw);
				if (username !== "everyone" && (!scores[username] || scores[username].average === void 0)) {
					msg.send(`${String(username)} has no happiness average yet`);
				} else {
					for (user in scores) {
						data = scores[user];
						if ((user === username || username === "everyone") && data.average !== void 0) {
							msg.send(`${String(user)} has a happiness average of ${data.average}`);
						}
					}
				}
			} else {
