...
(function() {
	module.exports = function(robot) {
		let botname, minusminus_re, plusplus_minusminus_re, plusplus_re;
		...
		robot.hear(plusplus_minusminus_re, (msg) => {
			let count, match, res, sending_user, user;
		...
		robot.hear(RegExp(`${String(botname)}\\s+@([a-z0-9_\\-\\.]+)`, "i"), (msg) => {
			let count, point_label, user;
		...
		return robot.hear(RegExp(`${String(botname)}\\s+leaderboard`, "i"), (msg) => {
			let _i, _ref, i, leader, limit, newline, point_label, points, score, str, tuples, username, users;
			...
			str = "";
			limit = 5;
			for (i = _i = 0, _ref = Math.min(limit, tuples.length); _ref >= 0 ? _i < _ref : _i > _ref; i = _ref >= 0 ? ++_i : --_i) {
				username = tuples[i][0];
				points = tuples[i][1];
